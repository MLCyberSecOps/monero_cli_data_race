...
class SimpleMutexChecker:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            "recursive_mutex_decl": re.compile(
                r"(std::\s*)?recursive_mutex\s+(\w+)", re.IGNORECASE
            ),
            # Member variable mutexes
            "member_mutex": re.compile(
                r"\b(m_\w*mutex\w*|m_\w*lock\w*)\b", re.IGNORECASE
            ),
            # Mutex usage
            ...
            "global_var": re.compile(
                r"^\s*(?:extern\s+)?(?:static\s+)?(?:int|long|bool|char|float|double|size_t)\s+(\w+)",
                re.MULTILINE,
            ),
            ...
            "thread_creation": re.compile(
                r"(std::\s*)?thread\s+\w+|pthread_create|boost::\s*thread"
            ),
            # Atomic variables
                ...
                for match in matches:
                    ...
                    if mutex_name and mutex_name not in mutexes:
                        mutexes[mutex_name] = MutexInfo(
                            name=mutex_name, declared_line=i
                        )
    ...
    def _is_access_protected(
        self, lines: List[str], line_num: int, var_name: str
    ) -> bool:
        """
    ...
    def _analyze_protection(
        ...
        shared_vars: Dict[str, SharedVariable],
    ) -> Dict:
        ...
        protected_vars = sum(
            1 for v in shared_vars.values() if v.protection_ratio > 0.5
        )

        unprotected_accesses = []
        for var_name, var_info in shared_vars.items():
            if var_info.has_unprotected_access:
                unprotected_count = len(var_info.access_lines) - len(
                    var_info.protected_access_lines
                )
    ...
    def _generate_summary(
        self, mutexes: Dict, shared_vars: Dict, has_threading: bool
    ) -> Dict:
        ...
        used_mutexes = sum(1 for m in mutexes.values() if m.is_used)
        total_shared_vars = len(shared_vars)
        unprotected_vars = sum(
            1 for v in shared_vars.values() if v.has_unprotected_access
        )
...
def format_simple_report(result: Dict) -> str:
    ...
    report.append(f"\n📊 Shared Variables:")
    report.append(
        f"   • Total shared variables: {protection['total_shared_variables']}"
    )
    report.append(f"   • Protected variables: {protection['protected_variables']}")
    if protection["unprotected_variables"] > 0:
        report.append(
            f"   • ⚠️ Unprotected variables: {protection['unprotected_variables']}"
        )
    ...
    if result["mutexes"]:
        ...
        for mutex_name, mutex_info in result["mutexes"].items():
            ...
            report.append(
                f"   • {mutex_name} (line {mutex_info.declared_line}): {status}"
            )
    ...
    if protection["unprotected_accesses"]:
        ...
        for access in protection["unprotected_accesses"]:
            ...
            report.append(
                f"   • {access['variable']}: {access['unprotected_accesses']}/{access['total_accesses']} unprotected ({protection_pct:.0f}% protected)"
            )
