...
class ThreadGuardAnalyzer:
        ...
        for i, line in enumerate(lines, 1):
            ...
            for var_name, mutex_name in list(raii_vars.items()):
                if f"{var_name}." in line or f"{var_name}->" in line:
                    ...
                    continue
                if (
                    var_name in line and "=" not in line.split("//")[0]
                ):  # Not an assignment
    ...
    def _detect_deadlock_potential(self):
        ...
        for func, locks in function_locks.items():
            ...
            for lock in locks:
                if (
                    not lock[2] and lock[0] not in seen_locks
                ):  # Not recursive and not seen
        ...
        for func, locks in function_locks.items():
            ...
            all_ops = sorted(
                locks + [(call, 0, False) for call in function_calls.get(func, [])],
                key=lambda x: x[1],
            )
