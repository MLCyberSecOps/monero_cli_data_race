...
class EnhancedThreadGuardAnalyzer:
    ...
    def _compile_enhanced_patterns(self):
        ...
        self.enhanced_patterns = {
            ...
            'map_declaration': re.compile(r'std\s*::\s*(?:unordered_)?map\s*<[^>]+>\s+(\w+)'),
            'set_declaration': re.compile(r'std\s*::\s*(?:unordered_)?set\s*<[^>]+>\s+(\w+)'),
            'string_declaration': re.compile(r'std\s*::\s*(?:w)?string\s+(\w+)'),
            
            # Container method calls
            'container_method': re.compile(r'(\w+)\.(push_back|insert|erase|clear|resize|at|operator\[\]|\[\])\s*\('),
            'container_iterator': re.compile(r'(\w+)\.(begin|end|rbegin|rend|find|lower_bound|upper_bound)\s*\('),
    ...
    def analyze_control_flow_paths(self, content: str, filename: str):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if method_match:
                ...
                self._analyze_function_control_flow(lines, i, current_function, current_class)
                continue
    
    def _analyze_function_control_flow(self, lines: List[str], start_line: int, 
                                     function_name: str, class_name: str):
        """
    ...
    def _extract_control_flow_paths(self, function_lines: List[Tuple[int, str]], 
                                   function_name: str) -> List[ControlFlowPath]:
        """
        ...
        for line_num, line in function_lines:
            ...
            if self.enhanced_patterns['early_return'].search(stripped):
                ...
                unreleased = set(current_path.locks_acquired) - set(current_path.locks_released)
                if unreleased:
    ...
    def _check_path_for_lock_issues(self, path: ControlFlowPath, 
                                   function_name: str, class_name: str):
        """
        ...
        if unreleased and path.has_early_exit:
            for mutex in unreleased:
                issue = (f"Missing unlock for '{mutex}' in {function_name}() "
    ...
    def analyze_stl_container_usage(self, content: str, filename: str):
        ...
        for i, line in enumerate(lines, 1):
            ...
            container_method_match = self.enhanced_patterns['container_method'].search(line)
    ...
    def _is_container_operation_thread_safe(self, container_type: str, method: str) -> bool:
        """
        Determine if a container operation is thread-safe
        """
        # Most STL containers are NOT thread-safe for write operations
        write_operations = {
            'push_back', 'insert', 'erase', 'clear', 'resize', 'pop_back',
            'pop_front', 'assign', 'swap', 'operator[]'
        }
        
        read_operations = {
            'size', 'empty', 'at', 'front', 'back', 'begin', 'end',
    ...
    def _check_container_synchronization(self, access: STLContainerAccess, 
        ...
        sync_patterns = [
            r'lock_guard', r'unique_lock', r'shared_lock', r'scoped_lock',
            r'\.lock\s*\(', r'\.unlock\s*\(', r'std::atomic',
            r'mutex', r'critical_section'
        ]
        
        has_synchronization = any(re.search(pattern, context, re.IGNORECASE) 
                                for pattern in sync_patterns)
        
        if not has_synchronization:
            issue = (f"Unsynchronized access to STL container '{access.container_name}' "
                        ...
                        for held_lock in locks_held:
                            for potential_lock in potential_locks:
                                if held_lock != potential_lock:
                                    ...
                                    issue = (f"Potential deadlock in {class_name}::{method_name}() "
    ...
    def _get_potential_locks_in_call_chain(self, class_name: str, method_name: str, 
                                         class_methods: Dict, visited: Set[str], 
                                         depth: int = 0) -> Set[str]:
        """
...
if __name__ == "__main__":
    ...
    for filename, content in test_cases.items():
        ...
        if not enhancer.result.deadlock_risks and not enhancer.result.inconsistent_locking:
