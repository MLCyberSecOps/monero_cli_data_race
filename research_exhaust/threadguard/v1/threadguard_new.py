...
class ThreadGuardAnalyzer:
    ...
    def __init__(self):
        ...
        # Thread entry points
        self.thread_entry_points = {"reader_thread_func", "run", "start"}
    ...
    def analyze_file(self, filepath: Path) -> AnalysisResult:
        ...
        try:
            ...
            self.result.inconsistent_locking.append(f"ERROR during analysis: {str(e)}")
            ...
            if in_async_stdin_reader:
                ...
                if "m_read_status" in line:
                    ...
                    if not any(
                        ...
                        if any(
                            op in line
                            for op in ["if (", "while (", "switch (", "case "]
                        ):
    ...
    def _extract_shared_variables(self, content: str):
        ...
        header = content
        first_def = re.search(
            r"^\s*(?:class\s+\w+|[\w:<>]+\s+\w+\s*\()", content, re.MULTILINE
        )
        if first_def:
        ...
        for i, line in enumerate(lines, 1):
            ...
            for var in self.shared_vars:
                if var in line:
                    access_type = (
                        AccessType.WRITE
                        if "=" in line.split(var)[0]
                        else AccessType.READ
                    )
                    self.memory_accesses[var].append(
                        MemoryAccess(
                            variable=var,
    ...
    def _extract_locking_patterns(self, content: str, filename: str):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (
                func_match
                and "{" in line
                and not any(kw in line for kw in ["if", "while", "for", "switch"])
            ):
                current_function = func_match.group(1)
                scope_stack.append(
                    {
            ...
            if "}" in line and scope_stack:
                ...
                # Add implicit unlocks for all locks in this scope
                for mutex_name in current_scope["locks_held"]:
                    self.locking_patterns[mutex_name].append(
                        LockingPattern(
                            mutex_name=mutex_name,
                            operation="unlock",
                            line_number=i,
                            function=current_scope["function"]
                            if current_scope["function"]
                            else "<global>",
                        )
                    )
                scope_stack.pop()  # Remove the ended scope from stack
            ...
            lock_guard_match = re.search(
                r"std\s*::\s*lock_guard\s*<[^>]+>\s+(\w+)\s*\(\s*([^)]+)\s*\)", line
            )
            if lock_guard_match:
            ...
            # Track RAII variables in current scope
            if (
                scope_stack
                and "{" in line
                and not any(kw in line for kw in ["if", "while", "for", "switch"])
            ):
                # New scope started, push to stack with empty locks set
                scope_stack.append(
                    {
                        "start_line": i,
                        "end_line": None,
            ...
            # Remove RAII variables when they go out of scope
            if "}" in line and scope_stack:
                # We already handled the unlocks for this scope above
                # Just clean up any RAII variables that might have been in this scope
                current_scope = scope_stack[-1] if scope_stack else None
                if current_scope:
                    for var_name, mutex_name in list(raii_vars.items()):
                        if mutex_name in current_scope["locks_held"]:
                ...
                for i in range(len(accesses)):
                    for j in range(i + 1, len(accesses)):
                        ...
                        if var in self.global_vars:
                            ...
                            if (
                                access1.thread_context != access2.thread_context
                                and not (access1.is_protected and access2.is_protected)
                            ):
                                self.result.races.append(
                                    RaceCondition(
                                        variable=var,
    ...
    def _analyze_locking_consistency(self):
        ...
        print("\n=== Debug: Analyzing Locking Consistency ===")
        for mutex, operations in self.locking_patterns.items():
            ...
            for i, op in enumerate(operations):
                try:
                    print(
                        f"  Op {i}: {op.operation} in {op.function}() at line {op.line_number}"
                    )
                    ...
                    if op.operation == "lock":
                        ...
                        print(f"    Lock count: {lock_count}")
                        if lock_count < 0:
                            msg = f"Unlock without matching lock for {mutex} in {op.function}() at line {op.line_number}"
                            print(f"    WARNING: {msg}")
                            self.result.inconsistent_locking.append(msg)
                            lock_count = 0
                except Exception as e:
                    print(f"ERROR processing operation {i}: {e}")
                    print(f"Operation: {op}")
                    print(f"Operation type: {type(op)}")
                    print(
                        f"Operation dict: {op.__dict__ if hasattr(op, '__dict__') else 'No __dict__'}"
                    )
                    raise

            if lock_count > 0:
                msg = f"Potential lock leak for {mutex} - {lock_count} lock(s) not released"
                print(f"  WARNING: {msg}")
                self.result.inconsistent_locking.append(msg)

        print("=== End of Locking Consistency Analysis ===\n")
    ...
    def _detect_deadlock_potential(self):
        ...
        function_locks = defaultdict(
            list
        )  # function_name -> list of (lock_name, line_number, is_recursive, is_raii)
        ...
        for mutex, operations in self.locking_patterns.items():
            ...
            for func, ops in func_ops.items():
                ...
                for op in sorted_ops:
                    if op.operation == "lock":
                        is_raii = "lock_guard" in str(
                            op
                        )  # Check if this is an RAII-style lock
                        if mutex in acquired:
                            # Mark as recursive lock
                            function_locks[func].append(
                                (mutex, op.line_number, True, is_raii)
                            )
                            ...
                            function_locks[func].append(
                                (mutex, op.line_number, False, is_raii)
                            )

                        # Track lock acquisition with RAII info
                        acquired[mutex] = {"line": op.line_number, "is_raii": is_raii}
                        ...
                        if mutex in acquired:
                            ...
                            if not any(
                                "lock_guard" in str(op2)
                                for op2 in ops
                                if op2.operation == "lock"
                                and op2.line_number < op.line_number
                            ):
                                self.result.deadlock_risks.append(
        ...
        # Second pass: Build lock sequences and detect potential issues
        print("\n=== Function Lock Sequences ===")
        function_lock_sequences = {}

        # First, process all functions to get their lock sequences
        for func, locks in function_locks.items():
            # Get unique non-recursive locks in acquisition order
            seen_locks = set()
            unique_locks = []

            # Sort locks by line number to get acquisition order
            sorted_locks = sorted(locks, key=lambda x: x[1])

            for lock in sorted_locks:
                lock_name, line_num, is_recursive, is_raii = lock
                if (
                    not is_recursive and lock_name not in seen_locks
                ):  # Skip recursive locks and duplicates
                    seen_locks.add(lock_name)
                    unique_locks.append(lock_name)

            if (
                len(unique_locks) >= 1
            ):  # Track all functions that acquire at least one lock
                ...
                for i in range(len(unique_locks)):
                    for j in range(i + 1, len(unique_locks)):
                        ...
                        # Add edge from lock1 to lock2 (lock1 is acquired before lock2)
                        if not lock_graph.has_edge(lock1, lock2):
                            lock_graph.add_edge(lock1, lock2, functions=set())
                        lock_graph.edges[lock1, lock2]["functions"].add(func)

                        print(f"  - {lock1} -> {lock2} (in {func}())")

        # Third pass: Detect potential deadlock cycles
        print("\n=== Deadlock Cycle Detection ===")
        try:
            # Find all cycles in the lock graph
        ...
        for func, locks in function_locks.items():
            ...
            all_ops = []
            # Add lock operations (each lock is a tuple of (lock_name, line_number, is_recursive, is_raii))
            all_ops.extend(locks)
            # Add function calls as special operations (using a tuple with a different structure)
            for call in function_calls.get(func, []):
                all_ops.append(
                    (call, 0, False, False, True)
                )  # (func_name, line_num, is_recursive, is_raii, is_call)

            # Sort all operations by line number
            all_ops.sort(key=lambda x: x[1])

            for op in all_ops:
                # Handle both lock operations and function calls
                if len(op) == 5 and op[4]:  # This is a function call
                    func_name, line_num, _, _, _ = op
                    lock_name = func_name
                    is_recursive = False
                    is_call = True
                else:
                    lock_name, line_num, is_recursive, is_raii = op
                    is_call = False

                # If this is a function call, check if it could lead to a deadlock
                if is_call and lock_name in function_calls:
                    called_func = lock_name
                    if called_func in function_locks:
                        # Get all non-recursive locks acquired by the called function
                        called_locks = [
                            lock[0]
                            for lock in function_locks[called_func]
                            if not lock[2]
                        ]
    ...
    def _calculate_metrics(self):
        ...
        self.result.metrics = {
            ...
            "high_races": len(
                [r for r in self.result.races if r.severity == SeverityLevel.HIGH]
            ),
            "medium_races": len(
                [r for r in self.result.races if r.severity == SeverityLevel.MEDIUM]
            ),
            "low_races": len(
                [r for r in self.result.races if r.severity == SeverityLevel.LOW]
            ),
        ...
        if "reader_thread_func" in content:
            ...
            if read_loop:
                ...
                if (
                    ...
                    not in content[read_loop.start() : read_loop.start() + 500]
                ):
                    self.result.inconsistent_locking.append(
...
""",
    ...
    if args.ci_mode:
        critical_races = len(
            [r for r in result.races if r.severity == SeverityLevel.CRITICAL]
        )
        high_races = len([r for r in result.races if r.severity == SeverityLevel.HIGH])

        if critical_races > args.max_critical:
            print(
                f"❌ CI FAILURE: {critical_races} critical issues found (max allowed: {args.max_critical})"
            )
            sys.exit(2)
        elif high_races > args.max_high:
            print(
                f"⚠️  CI WARNING: {high_races} high severity issues found (max allowed: {args.max_high})"
            )
            sys.exit(1)
        else:
