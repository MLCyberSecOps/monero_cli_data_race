...
class ThreadGuardAnalyzer:
    ...
    def _extract_locking_patterns(self, content: str, filename: str):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if '}' in line and scope_stack:
                ...
                for mutex_name in current_scope['locks_held']:
                    self.locking_patterns[mutex_name].append(LockingPattern(
                        ...
                        operation='unlock',
                        line_number=i,
                        function=current_scope['function'] if current_scope['function'] else "<global>"
                    ))
            ...
            if scope_stack and '{' in line and not any(kw in line for kw in ['if', 'while', 'for', 'switch']):
    ...
    def _analyze_locking_consistency(self):
        ...
        for mutex, operations in self.locking_patterns.items():
            ...
            for i, op in enumerate(operations):
                try:
                    print(f"  Op {i}: {op.operation} in {op.function}() at line {op.line_number}")
                    ...
                    print(f"Operation dict: {op.__dict__ if hasattr(op, '__dict__') else 'No __dict__'}")
                    raise
    ...
    def _detect_deadlock_potential(self):
        ...
        for func, locks in function_locks.items():
            ...
            for lock in sorted_locks:
                ...
                if not is_recursive and lock_name not in seen_locks:  # Skip recursive locks and duplicates
                    seen_locks.add(lock_name)
                    unique_locks.append(lock_name)
            
            if len(unique_locks) >= 1:  # Track all functions that acquire at least one lock
        ...
        for func, locks in function_locks.items():
            ...
            for call in function_calls.get(func, []):
                all_ops.append((call, 0, False, False, True))  # (func_name, line_num, is_recursive, is_raii, is_call)
