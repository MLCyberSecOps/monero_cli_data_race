...
class RemoveLockMutator(CodeMutator):
    ...
    def mutate(self, code: str) -> str:
        lines = code.split('\n')
        lock_lines = [i for i, line in enumerate(lines) 
        ...
        num_mutations = random.randint(self.min_mutations, min(self.max_mutations, len(lock_lines)))
...
class SwapLockOrderMutator(CodeMutator):
    ...
    def mutate(self, code: str) -> str:
        ...
        for i, line in enumerate(lines):
            if re.search(r'std::(mutex|lock_guard|unique_lock|shared_lock|scoped_lock)', line):
...
def run_mutation_test(original_code: str, test_func: Callable, 
                     mutators: List[CodeMutator], num_iterations: int = 10) -> Dict[str, List[str]]:
    """
